...
const Update = () => {
    ...
    const { register, handleSubmit, setError, formState: { errors }, resetField, reset } = useForm<CreateCertificateRequest>(
        {
    ...
    const onSubmit = handleSubmit((data: CreateCertificateRequest) => {
        ...
        return certificateService.update(data).then((response: any) => {
            console.log(response);

            setValues({ ...values, message: `Certificate updated `, loading: false })
            reset()
        })
            .catch((error: any) => {
                console.log(error);
    ...
    return (
        <DatePickerWrapper>
            <Grid container spacing={6}>
                ...
                <Grid item xs={12}>
                    <Card>
                        <CardHeader title='Update a certifiacte' titleTypographyProps={{ variant: 'h6' }} />
                        {
                        ...
                        <form onSubmit={onSubmit}>
                            <CardContent>
                                <Grid container spacing={5}>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <TextField fullWidth label='Dob' type="date" placeholder='27/02/1999' {...register('dob')} helperText={errors.dob && errors.dob?.message} />
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
                                        <TextField fullWidth type='classification' label='Classification' placeholder='2.1' {...register('classification')} helperText={errors.classification && errors.classification?.message} />
                                    </Grid>
                                    <Grid item xs={12} sm={12}>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <TextField fullWidth label='Enrol' type="number" placeholder='2018' {...register('enrol')} helperText={errors.enrol && errors.enrol?.message} />
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
                                        <TextField fullWidth type='grad' label='Graduation' type="number" placeholder='2022' {...register('grad')} helperText={errors.grad && errors.grad?.message} />
                                    </Grid>
                                </Grid>
                            </CardContent>
                            <Divider sx={{ margin: 0 }} />
